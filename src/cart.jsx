...
function Products() {
  const { storeProducts, setStoreProducts, cartItems, setCartItems } =
    useStore();
    ...
    >
      ...
      <div style={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>
        <div className={homepageStyles.featuredProductsContainer}>
          <div className={cartStyles.featuredProducts}>
            {cartItems.items.length < 1 ? (
              <div className={`${homepageStyles.welcomeName} ${homepageStyles.oswald}`}>There is nothing in your cart!</div>
            ) : (
          ...
          <div></div>
          <div>Total Cost: ${cartItems.totalPrice}</div>
          <button className={homepageStyles.welcomeProducts}>Checkout</button>
        </div>
      </div>
